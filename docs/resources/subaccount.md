...
* `utilization_settings` - (Optional) Used to enable the option to log account utilization metrics and expected utilization at the current indexing rate, for the purpose of managing account utilization and avoiding running out of capacity. Send an `frequencyMinutes` to determine the sampling rate in minutes for measuring utilization metrics, and `utilizationEnabled` to enable the feature. [Learn more about managing account usage](https://docs.logz.io/user-guide/accounts/manage-account-usage.html)
